...
public final class SQLQuery {
    ...
    //Courses constant
    public static final String SQL_GET_ALL_COURSE_BY_USER = "SELECT * FROM schedule WHERE schedule.id_course IN (SELECT id_course FROM users_courses WHERE id_user = ?)";
    public static final String SQL_GET_STARTED_COURSES_BY_USER = "SELECT * FROM schedule WHERE progress LIKE 'Started' AND schedule.id_course IN (SELECT id_course FROM users_courses WHERE id_user = 1)";
    public static final String SQL_GET_PENDING_COURSES_BY_USER = "SELECT * FROM schedule WHERE progress LIKE 'Pending' AND schedule.id_course IN (SELECT id_course FROM users_courses WHERE id_user = 1)";
    public static final String SQL_GET_FINISHED_COURSES_BY_USER = "SELECT * FROM schedule WHERE progress LIKE 'Finished' AND schedule.id_course IN (SELECT id_course FROM users_courses WHERE id_user = 1)";
    ...
    public static final String SQL_GET_COURSES_BY_TEACHER = "SELECT s.id, c.name, s.mark,u2.name, s.progress FROM schedule s" +
